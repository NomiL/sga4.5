...
\title{Cohomologie \'Etale\thanks{r\'edig\'e par J. Boutot}}
...
note $j_! F$ le prolongement par $0$ de $F$ à $\bar X$ et on définit les 
...
exacte $0\to F_U \to F \to F_{X\setminus U} \to 0$ et, par récurrence sur $n$, 
...
Soient $f:X\to S$ un morphisme séparé de type fini et $F$ un faisceau 
constructible sur $X$. Alors les faisceaux $\R^q f_! F$ sont constructibles. 
\end{theorem}

Nous ne considérons que le cas où $F$ est annulé par un entier inversible 
sur $X$. 

Par démontrer le théorème on se ramène au cas o`u $F$ est un faisceau 
...
et $F$ un faisceau de torsion sur $X$. Notons par un exposant $\an{}$ le foncteur 
de passage aux espaces topologiques usuels, et par $\R^q \an f_!$ les foncteurs 
dérivés du foncteur image directe à support propre par $\an f$. On a 
\[
  \an{(\R^q f_! F)} \simeq \R^q \an f_! \an F \text{.}
\]
En particulier, pour $S=$ un point et $F$ le faisceau constant $\dZ/n$, 
\[
...
$F=\dZ/n$. Les groupes de cohomologie considérés sont alors nuls pour 
...
$F$ un faisceau de torsion sur $X$. Alors on a $\h^1(X,F) = 0$ pour 
$q>\dim(X)$. 
\end{theorem}
