...
\usepackage{
  ...
  microtype,
  stmaryrd
} %,titlesec} to have inline subsubsections
...
Nous renvoyons à \cite[XVI \S 4 et \S 5]{4} pour la démonstration des 
...
charactéristique $0$ dans \cite[XIX \S 5]{4}. Une autre démonstration, 
indépendante de la résolution, est donnée dans ce volume (Th. finitude, 
1.1),
...
Pour une preuve très économique, voir (Dualité \S 2).
...
\end{equation}
Cette compatibilité est prouvée en (Dualité, 3.2.4). 
...
certains des exposés donnés par Grothendieck \`a l'IHES au printemps 1966. Dans 
l'esprit de ce volume, il ne sera pas fait appel \`a SGA 5 -- saui deux 
références \`a das passages de l'exposé XI, indépendant au reste de ce 
séminaire. 
...
$p^n$ de $P$, on note $\dF_{p^n}$ le sous-corps \`a $p^n$ éléments de $\dF$. 
...
par extension des scalaires \`a $\dF$. Par exemple, si $\cF_0$ est un faisceau 
...
En cas d'ambiguite, on ecrira $F_{(q)}$ plutot que $F$. 
...
\section{Correspondance de Frobenius}
...
Pour une définition en forme, je renvoie \`a SGA 5 XI \S 1,2. On n'y considère 
que le cas o\`u $q=p$, mais le cas général se traite de même. 
...
\chapter{La classe de cohomologie associée \`a un cycle}\label{IV}
